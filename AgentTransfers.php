<?php
	...
	$transfersQuery = "SELECT fromDirectorID, toDirectorID, price, status, playerID 
						FROM Transfer_Offer 
						WHERE Transfer_Offer.playerID IN (SELECT ID FROM Player WHERE Player.agent_ID = '".$agentID."')";
						...
						<tr>
							...
							<td>
								<?php
									...
									$curStatus = '0';
									if ($statuses[$cnt] == 'Director Accepted')
										$curStatus = '2'; 
									if ($statuses[$cnt] == 'Requested' || $statuses[$cnt] == 'Director Accepted') { ?>
										<form action = "#" method = "POST">
									...
									<?php } ?>
									<?php if ($statuses[$cnt] == 'Agent Accepted' || $statuses[$cnt] == 'Transfer Completed') { 
											echo "N/A";
									} ?>
							</td>
						</tr>
