	...
	if (isset($_POST['request'])){
		if (empty($_POST['name']) || empty ($_POST['surname']) || empty ($_POST['bonus']) || empty ($_POST['date'])){
			...
			if (mysqli_num_rows($getPlayerID) == 0){
				...
				$checkNameQuery = "SELECT * FROM Player WHERE ID = '".$curPlayerID->ID."' AND agent_ID = '".$_SESSION['id']."'";
				$checkName = mysqli_query($connection, $checkNameQuery);
				if (mysqli_num_rows($checkName) == 0){
					?>
					<script>alert("Choose YOUR clients"); </script>
					<?php
				}
				else{
					if (!is_numeric($bonus)){
						...
						$existQuery = "SELECT * FROM Contract WHERE playerID = '".$curPlayerID->ID."' AND agentID = '".$_SESSION['id']."' AND status <> '2'";
						...
						if (mysqli_num_rows($exist) > 0){
							...
							$getDirectorIDQuery = "SELECT Director.ID FROM Director, Plays WHERE Plays.clubID = Director.club_ID AND Plays.playerID = '".$curPlayerID->ID."'";
							$getDirectorID = mysqli_query($connection, $getDirectorIDQuery)->fetch_object();
							$updateQuery = "INSERT INTO Contract(playerID, directorID, agentID, bonus, expirationDate, status) 
											VALUES('".$curPlayerID->ID."', '".$getDirectorID->ID."', '".$_SESSION['id']."', '".$bonus."', '".$date."', '0')";
