<?php 
	...
	if (isset($_POST['accept'])){
		...
		$updateQuery = "UPDATE Contract SET status = '2' WHERE playerID = '".$playerID."' AND agentID = '".$agentID."'";
	...
	$contractsQuery = "SELECT playerID, expirationDate, bonus, status, playerID, agentID FROM Contract WHERE directorID = '".$curDirectorID."'";
	...
	while ($row = mysqli_fetch_assoc($contracts)){ 
		...
		array_push($agentIDs, $row['agentID']);
		array_push($bonuses, $row['bonus']);
		
		if ($row['status'] == '0'){
			$curStatus = "Agent Requested";
		}
						...
						<tr>
							...
							<td>
								<?php
									...
									$curAgentID = $agentIDs[$cnt];
									if ($statuses[$cnt] == 'Agent Requested') { ?>
										<form action = "#" method = "POST">
											<input type = "hidden" name = "id1" value = "<?=$curPlayerID?>">
											<input type = "hidden" name = "id2" value = "<?=$curAgentID?>">
